...
`isolated_iter()` snapshots `torch.get_rng_state()` (and every visible CUDA device's RNG state) once, before consuming the loader at all. It is both a plain iterator (drop-in for `for batch in isolated_iter(loader): ...`) and a context manager: `with isolated_iter(loader) as batches: ...` gives a *deterministic* restoration guarantee via `__exit__` — restoration no longer depends on the returned object being closed, exhausted, or garbage collected. Prefer the `with` form whenever the restoration guarantee matters, especially for manual `next()`-driven consumption (prefetch/double-buffering loops); the bare `for` idiom still works and restores reliably under CPython's refcounting once the loop exits by any means, but a caller that instead stores the iterator in a variable and drives it with `next()` across exception handling does not get a synchronous guarantee without `with`. Both DataLoader-iterator construction and per-batch `shuffle=True` permutation draws are neutralized, since both were independently confirmed to consume global RNG state on this host (see Scope and limitations). Restoration also fires if `iter(dataloader)` itself raises after already mutating RNG state, and if the wrapped iterator's own `__next__` raises mid-consumption (e.g. a worker crash) — not just on normal exhaustion.

## Scope and limitations

The probe covers `shuffle=False` and `shuffle=True` with `num_workers=0` (in-process) on CPU tensors. The leak is documented upstream as independent of `num_workers`, but `num_workers>0` multiprocessing worker-seed behavior is not separately re-verified by this tool — treat that as untested, not confirmed-safe. This tool does not audit `IterDataPipe`/`torchdata` pipelines, distributed samplers, or CUDA-specific RNG paths beyond the isolated_iter() CUDA-state save/restore itself. As of v0.2.0, restoration on the bare (non-`with`) iterator form still relies on the object being closed, exhausted, or garbage collected — a caller that stores the iterator in a variable and drives it manually with `next()` while holding a live reference across exception handling does not get a synchronous guarantee that way; use `with isolated_iter(loader) as batches:` when the restoration guarantee matters, not just the plain iterator form.
